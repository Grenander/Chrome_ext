...
const handleBackgroundResponse = response => {
    ...
    // Call the filter before rendering...
    filter();
}
...
/************************************************************************************
 * Added by Johny Grenander - extract and modify elements...
 ************************************************************************************/

 function filter() {
    //let x = document.getElementsByClassName("logo");
    let x = document.getElementById("megaAd-placehoder");
  
    for (let i = 0; i < x.length; i++) {
      let y = window.getSelection(x[i]);

      console.log('Found: '+i);

      /*let style = window.getComputedStyle(x[i]);
